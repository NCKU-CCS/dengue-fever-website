...
if __name__=='__main__':
    url_t = "http://e-service.cwb.gov.tw/HistoryDataQuery/DayDataController.do?command=viewMain&station=467440&datepicker="
    now = datetime.strptime('2015/09/01', '%Y/%m/%d').date() + timedelta(days=10)
    end = datetime.now().date() #+ timedelta(days=1)
    data = json_io.read_json('../data/dynamic/weather.json')
    while now < end:
        print (now)
    ...
    json_io.write_json('../data/dynamic/weather.json', data)
